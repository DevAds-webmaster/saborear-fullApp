...
export function ItemModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        {featuredText && <div className="mb-2 font-bold" style={{ color: featuredTextColor || "#0f0" }}>{featuredText}</div>}
        ...
        {glutenFree === "si" && <span className={`bg-yellow-300 p-1 rounded-md float-right font-bold text-xs uppercase mx-1 ${style?.modalsItemsStyles.tagsTextColor}`}>Sin TACC</span>}
        {veggie === "si" && <span className={`bg-green-600 p-1 rounded-md float-right font-bold text-xs text-amber-50 uppercase mx-1 ${style?.modalsItemsStyles.tagsTextColor}`}>Veggie</span>}
...
export function ParamModal({
  ...
  codePromMod,
  featuredTextMod,
  featuredTextColorMod,
}: any) {
  if (!open) return null;
  if (enableMod !== "si") return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div
        ...
        onClick={(e) => e.stopPropagation()}
      >
        {featuredTextMod && <div className="mb-2 font-bold" style={{ color: featuredTextColorMod || "#0f0" }}>{featuredTextMod}</div>}
        {imageMod && <img src={imageMod} alt={titleMod} className="w-full h-56 object-cover rounded mb-4" />}
        <div className="font-bold italic text-xl mb-2">{titleMod}</div>
        <div className="mb-4">{descriptionMod}</div>
        {codePromMod && <span className="bg-gray-800 p-1 rounded-md text-center font-bold text-md text-amber-50 uppercase mx-1">{codePromMod}</span>}
      </div>
    </div>
  );
}
