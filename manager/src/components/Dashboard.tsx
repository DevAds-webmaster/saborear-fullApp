...
export default function Dashboard({
  ...
  const startEdit = (u: ManagerUser) => {
    setEditingUserId(u.id);
    setEditForm({
      username: u.username,
      email: u.email,
      role: u.role,
      resto: visibleRestoId(u) || "",
    });
  };
  ...
  const resetPassword = async (u: ManagerUser) => {
    ...
    try {
      setSaving(true);
      const message = await usersService.resetUserPassword(u.id, password);
      setSuccess(message);
    } catch (err) {
      setError((err as Error).message);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        <header className="bg-white rounded-lg border p-4 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 flex-1">
            ...
            {envError && <p className="mt-2 text-sm text-red-600 font-medium">{envError}</p>}
          </div>
          <div className="flex flex-col items-stretch sm:items-end gap-2 shrink-0">
            <span className="text-sm text-gray-600 truncate max-w-full text-right" title={user.email}>
              {user.email}
            </span>
            <button
              type="button"
                  ...
                  {groupedUsers.map(({ groupKey, groupSlug, users: groupList }) => (
                    <Fragment key={groupKey}>
                      ...
                      {groupList.map((u) => (
                        <tr
                          key={u.id}
                          className={`border-b last:border-b-0 ${u.role === "admin" ? "bg-gray-100" : ""}`}
                        >
        ...
        <section className="bg-white rounded-lg border p-4">
          ...
          {orphanSectionError && <p className="mt-3 text-sm text-red-600">{orphanSectionError}</p>}
          {purgeResultMessage && <p className="mt-3 text-sm text-green-700">{purgeResultMessage}</p>}
        </section>
      </div>
    </div>
  );
